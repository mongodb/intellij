...
class ComponentHolder(val components: MutableList<Component>)
class NodeHolder(val nodes: MutableList<Node<Unit>>)
...
object ModelDsl {
    ...
    inline fun <reified T : Any> ComponentHolder.constant(value: T) {
        components.add(
            HasValueReference(
                HasValueReference.Constant(Unit, value, value.toBsonType())
            )
        )
    }
